...
import { Button, Col, Form, Grid, Input, Modal, notification, Row, Select, Table, Tag, Tooltip, Popover, Badge, Divider } from "antd";
...
const Payments = ({ onSelectPayment }) => {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      await push(ref(db, `users/${userId}/payments`), values);
      form.resetFields();
  ...
  return (
    <div className={`payments-container ${darkMode ? "dark-mode" : ""}`} style={{ 
      ...
      <div className="payments-header" style={{
        ...
        <h2 className="payments-title" style={{ 
          margin: 0,
          fontSize: screens.xs ? '1.5rem' : '1.8rem',
          color: darkMode ? "#f1f1f1" : "inherit"
        }}>Payments List</h2>
        <div className="payments-actions" style={{
          display: 'flex',
          flexDirection: screens.xs ? 'column' : 'row',
          gap: '12px',
          width: screens.xs ? '100%' : 'auto'
      ...
      <style>{`
        ...
        .dark-table .ant-table-tbody > tr > td {
          background-color: #1f极速赛车开奖结果记录官网
1f1f !important;
          color: #f1f1f1 !important;
          border-bottom-color: #444 !important;
        }
